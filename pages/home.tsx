...
var data_stream: React.JSX.Element[] = [];
...
export async function getStaticProps() {
  ...
  for(var i in files){
    if(path.extname(files[i]) === ".mdx"){

      var file = fs.readFileSync( "_posts/" + path.parse(files[i]).name + ".json", "utf-8" );
      var result_2 = JSON.parse(file);

      result.push( "article/" + path.parse( files[i]).name , path.parse(files[i]).name + ".jpg", result_2.title, result_2.description );
